...
NSArray *fetchDecryptedKeys() {
    NSURL *url = [NSURL URLWithString:@"https://raw.githubusercontent.com/installdot/proxy/refs/heads/main/key.txt"];
    NSError *error = nil;
...
void showFileMenu() {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        UIAlertAction *copyAction = [UIAlertAction actionWithTitle:@"Copy to Document"
                                                             style:UIAlertActionStyleDefault
                                                           handler:^(UIAlertAction *action) {
            NSString *destPath = [[getDocumentsPath() stringByAppendingPathComponent:@"com.ChillyRoom.DungeonShooter.plist"] copy];
            NSError *error = nil;
...
// ---------------------------
// Gesture Recognizer for Triple-Finger Double-Tap
// ---------------------------
@interface CustomGestureRecognizer : UIGestureRecognizer
@end

@implementation CustomGestureRecognizer
// The following minimal implementation uses the built‑in properties.
@end
...
- (void)applicationDidBecomeActive:(UIApplication *)application {
    ...
    dispatch_once(&onceToken, ^{
        ...
        // Create a UITapGestureRecognizer with required properties.
        UITapGestureRecognizer *gestureRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:[NSBlockOperation blockOperationWithBlock:^{
            showFileMenu();
